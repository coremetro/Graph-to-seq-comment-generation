...
class graph2gru(nn.Module):
    def __init__(self, config, vocab, use_cuda, use_copy, use_bert, word_level_model, graph_model, pretrain=None):
        ...
        if use_copy:
            ...
            self.decoder = models.rnn_decoder(config, self.vocab_size, embedding=self.embedding, gru=True)
        # GRU不需要c神经元，因为是两层rnn模型,所以在这里将encoder的hidden output通过线性变换从decoder_hidden_size变为2倍
    ...
    def encode(self, contents, contents_mask, concepts, concept_mask, title_index, adjs):
        #  encoder 首先是memory_network, menory_network分为两个部分，第一个部分是bert(word_level_model),
        # bert 实际上是有两个multi-attention+前馈模型组成，每个multi-attention有4个head
        # contents的维度是[32,13,100]   32是batch_size,13是concept的个数，也是句子的个数，100是句子的长度
        #  每个content是一篇文章，维度是[13,100], bert的输入值是[13,101,128] ,101是把concept和content拼接在一起，然后embdding
        # 成128维 , 在mutli-attention中做一些变换，然后有残差相加的前馈网络，最后输出值还是[13,101,128]
        # 总而言儿，就是Transformer《Attention Is All You Need》中的模型
        # 第二部分是 gcn，只取了bert输出值的每个句子的第一个单词，维度是[13,128]传入gcn中,gcn的输出context是[13,256]
        # t_index是标题的序号，可以这样理解，state相当于，只取了标题的向量
        # state是[256],最后stack成[256*32]
        contexts = []
        states = []
        ...
        # else:
        #     contexts, attn = self.encoder.encode(contexts, concept_mask)
    ...
    def forward(self, batch, use_cuda):
        ...
        # 注释掉c0
        h0 = self.build_init_state(state, self.config.num_layers)
        if self.use_copy:
            ...
            outputs, final_state, attns = self.decoder(tgt[:, :-1], h0, contexts)
    ...
    def sample(self, batch, use_cuda):
        ...
        h0 = self.build_init_state(state, self.config.num_layers)
        if self.use_copy:
            sample_ids, final_outputs, p_gens = self.decoder.sample([bos], h0, contexts, concept_mask,
                                                                    title_index,
                                                                    max_oov=0, extend_vocab=concept_vocab)
        else:
            # sample_ids, final_outputs = self.decoder.sample([bos], (c0, h0), contexts)
            sample_ids, final_outputs = self.decoder.sample([bos], h0, contexts)
        probs, attns_weight = final_outputs
        alignments = attns_weight.max(dim=2)[1]
        # 返回alignments,而不是attns
