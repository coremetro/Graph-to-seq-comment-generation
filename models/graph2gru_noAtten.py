...
class graph2gru_noAtten(nn.Module):
    def __init__(self, config, vocab, use_cuda, use_copy, use_bert, word_level_model, graph_model, pretrain=None):
        ...
        if use_copy:
            ...
            self.decoder = models.rnn_decoder_noAtten(config, self.vocab_size, embedding=self.embedding)
    ...
    def encode(self, contents, contents_mask, concepts, concept_mask, title_index, adjs):
        # contexts是encoder outputs 没有attention机制时，不需将contexts传给decoder, 所以将全部contexts注释掉
        # contexts = []
        states = []
